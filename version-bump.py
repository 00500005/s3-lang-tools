...
parser.add_argument('--version', '-v',
                    help='manually specified version (overrides --bump)')
parser.add_argument('--packagejson', '-o',
                    help='target package.json to modify', default='package.json')


def main(args):
    if not args.dryrun:
        ensure_package_has_no_changes()
    version = args.version if args.version else get_bumped_version(
        args.bump, args.packagejson)
...
def ensure_package_has_no_changes():
    ...
    if output and len(output) > 0:
        raise Exception(
            'Commit or stash your changes before bumping the version' + '\n\n' + output)
