...
import { EndIndex, StartIndex, Token as GenericToken, TokenType as AllTokenTypes } from "./common";
...
export namespace Twinemarkup {
  ...
  export class Builder<SELF extends Builder<SELF, P>, P> extends GenericChainableTokenBuilder<SELF, AST.TwinemarkupToken, P> {
    ...
    setMarkupLink(link?: string | AST.Token[]): SELF {
      this.args.markupLink = link;
      return <SELF><Builder.Chainable<P>>this;
    }
    setMarkupTitle(title?: string | AST.Token[]): SELF {
    ...
    setImagePath(path?: string | AST.Token[]): SELF {
...
export namespace Variable {
  ...
  export class Builder<SELF extends Builder<SELF, P>, P> extends GenericChainableTokenBuilder<SELF, AST.VariableToken, P> {
    ...
    addVariableNameParts(...parts: AST.Token[]): SELF {
      this.args.variableName
      this.args.variableName.splice(
        this.args.variableName.length - 1, 0, ...<VariableNameBuilderValue[]>parts);
      return <SELF><Builder.Chainable<P>>this;
    }
...
type MacroArgBuilderValue = AST.Token | { build: () => AST.Token }
...
class TokenBuilderArgs {
  ...
  markupLink?: string | AST.Token[];
  markupTitle?: string | AST.Token[];
  markupSetter?: AST.TwinescriptToken | Twinescript.Builder.Generic;
  markupImgPath?: string | AST.Token[];
  ...
  build(): GenericToken {
    ...
    switch (this.tokenType) {
      case AllTokenTypes.Content:
        return <AST.ContentToken>standard;
