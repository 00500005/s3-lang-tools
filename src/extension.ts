...
function serverOptions(context: ExtensionContext) : ServerOptions {
	...
	let prodModule = context.asAbsolutePath(
		path.join('dist', 'server.js')
	);
	let devModule = context.asAbsolutePath(
		path.join('out', 'server.js')
	);
	console.log('Got server module path', prodModule)
	...
	let serverOptions: ServerOptions = {
		run: { module: prodModule, transport: TransportKind.ipc, args: defaultArgs,  },
		debug: {
			module: devModule,
